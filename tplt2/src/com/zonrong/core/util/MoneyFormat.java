...
public class MoneyFormat {
	private final String[] pattern = { "零", "壹", "贰", "叁", "肆", "伍", "陆", "柒",
			"捌", "玖" };

	private final String[] cPattern = { "", "拾", "佰", "仟", "万", "拾", "佰", "仟",
			"亿" };
			...
			if (fs.indexOf("00") == -1 || fs.indexOf("00") >= 2) {// 若前两位小数全为零，则跳过操作
				...
				for (int j = 0; j < fraction.length(); j++) {
					fraction.replace(j, j + 1,
							this.pattern[fraction.charAt(j) - 48]); // 替换大写汉字
				}
	...
	public static void main(String[] ar) {
		...
		System.out.println(new MoneyFormat().format("1234567890.1254"));
		System.out.println(new MoneyFormat().format(new Double(1234567890.1254).toString()));
	}
}
