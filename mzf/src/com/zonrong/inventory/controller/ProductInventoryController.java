...
import com.zonrong.inventory.service.TemporaryInventoryService;
	...
	public Map deliveryByProduct(@PathVariable final int productId,@RequestBody final Map<String, Object> params, HttpServletRequest request) {
		OperateTemplete templete = new HttpTemplete(request) {
			protected void doSomething() throws BusinessException {
				...
				productInventoryService.deliveryByProductId(MzfEnum.BizType.delivery, productId, remark, InventoryStatus.valueOf(priorStatus), this.getUser());
			}
		};
		return templete.operate();
	}
