...
public class MaintainService extends BillStatusService<MaintainStatus>{
    ...
    public int createMaintain(Map<String, Object> maintain, IUser user) throws BusinessException {
        ...
        if(StringUtils.isBlank(MapUtils.getString(maintain,"maintainMode"))){
            throw new BusinessException("维修方式不能有空");
        }
        ...
        if(list.size() >= 1){
            throw new BusinessException("维修条码：["+ maintainCode +"]重复" );
        }
    ...
    public void over(int maintainId, Map<String, Object> over, IUser user) throws BusinessException {
        ...
        //结算规则：“总部维修”的维修单只能由总部操作员结算，其他的维修单只能由开单门店操作员进行结算
        String maintainMode = MapUtils.getString(maintain,"maintainMode");
        if("总部维修".equalsIgnoreCase(maintainMode)){
            if(user.getOrgId()!=mzfOrgService.getHQOrgId()){
                throw new BusinessException("总部维修 的维修单只能由总部操作员完成结算操作。");
            }
        }else{
            if(user.getOrgId() != maintainOrgId){
                throw new BusinessException("非总部维修 的维修单只能由开单门店的操作员完成结算操作。");
            }
        }
