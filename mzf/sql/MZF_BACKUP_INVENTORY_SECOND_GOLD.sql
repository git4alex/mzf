IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MZF_BACKUP_INVENTORY_SECOND_GOLD]') AND type in (N'P', N'PC'))
    DROP PROCEDURE [dbo].[MZF_BACKUP_INVENTORY_SECOND_GOLD]
GO

CREATE PROC [dbo].[MZF_BACKUP_INVENTORY_SECOND_GOLD] AS
declare @tableName varchar(100),  @sql varchar(800)
set @tableName = 'HIST_INVENTORY_SECOND_GOLD_'+replace(convert(varchar(7),getDate(),23),'-','')
...
if object_id(@tableName,'u') is not null begin
    ...
    set @sql = 'INSERT INTO '+@tableName+'(TARGET_ID,TARGET_NUM,COST,GOLD_CLASS,WEIGHT,SOURCE,SOURCE_ID,CDATE)'
               ...
               +' WHERE i.TARGET_TYPE=''secondGold'''
end
else begin
